    ...
    OptionsPanel::OptionsPanel(sf::String("Dev"))
    {
        ...
        frameRateBox->onTextChange([&](){
            if(!frameRateBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::FRAME_RATE, frameRateBox->getText().toStdString());
        });
        ...
        ballSpeedBox->onTextChange([&](){
            if(!ballSpeedBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::BALL_SPEED, ballSpeedBox->getText().toStdString());
        });
        ...
        paddleSpeedBox->onTextChange([&](){
            if(!paddleSpeedBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::PADDLE_SPEED, paddleSpeedBox->getText().toStdString());
        });
        ...
        wallThicknessBox->onTextChange([&](){
            if(!wallThicknessBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::WALL_THICKNESS, wallThicknessBox->getText().toStdString());
        });
        ...
        ballRadiusBox->onTextChange([&](){
            if(!ballRadiusBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::BALL_RADIUS, ballRadiusBox->getText().toStdString());
        });
        ...
        aabb = tgui::CheckBox::create("");
                ...
                aabb->onCheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_AABB, true);
        });
                aabb->onUncheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_AABB, false);
                });
        ...
        shapes->onCheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_SHAPE, true);
        });
        shapes->onUncheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_SHAPE, false);
        });
        ...
        joints->onCheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_JOINTS, true);
        });
        joints->onUncheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_JOINTS, false);
        });
        ...
        centerOfMass->onCheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_MASS, true);
        });
        centerOfMass->onUncheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_MASS, false);
        });
        ...
        pair->onCheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_PAIRS, true);
        });
        pair->onUncheck([&](){
            EventManager::inst().post<GU::Evt::OnCheck>(nullptr, Gui::id::DEBUG_PAIRS, false);
        });
        ...
        velocityIterationsBox->onTextChange([&](){
            if(!velocityIterationsBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::VELOCITY_ITERATIONS, velocityIterationsBox->getText().toStdString());
        });
        ...
        positionIterationsBox->onTextChange([&]()
        {
            if(!positionIterationsBox->getText().empty())
                EventManager::inst().post<GU::Evt::OnTextChanged>(nullptr, Gui::id::POSITION_ITERATIONS, positionIterationsBox->getText().toStdString());
        });
